...
1. `AGENTS.md`：仓库架构、目录、场景和约定。
2. `Assets/_Project/PCG/README.md`：当前 PCG Lab 的玩法、生成规则、遥测、数据集和 ML-Agents 契约。
3. `Training/README.md`：Python 训练环境与启动方式。
4. `CHANGELOG.md`：最近 PCG、Game AI 和 ML-Agents 功能的完整验证记录。
...
必须读取 XML 和日志确认总数、失败数与错误堆栈，不要只依赖进程退出码。最后一次记录在 `CHANGELOG.md` 的结果是 26 passed、0 failed，但本次交付环境因为找不到 Unity executable，没有重新运行测试。
...
- 仓库没有 CI 配置。
- 仓库中未找到调用 `BuildPipeline.BuildPlayer` 的 Editor 构建方法；AGENTS.md 中的 WebGL CLI 命令仍含 `<buildmethod>` 占位符。要稳定执行 WebGL CLI 构建，需先实现一个 Editor-only 静态构建入口，并让失败通过异常或非零 `EditorApplication.Exit` 返回。
...
- 不提交 `Library/`、`Temp/`、`Logs/`、`.venv-mlagents/`、训练结果或本地智能体权限文件。

## 可直接交给 Grok 的启动提示

```text
你正在接手 D:\unity-game\3D-Platformer。先阅读 AGENTS.md、GROK_HANDOFF.md、
Assets/_Project/PCG/README.md、Training/README.md、CHANGELOG.md，以及
